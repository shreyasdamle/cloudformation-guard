...
use crate::rules::exprs::{
    ...
    WhenConditions, WhenGuardClause,
};
...
impl<'loc> std::fmt::Display for GuardAccessClause<'loc> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_fmt(format_args!(
            ...
            if self.access_clause.comparator.1 {
                "NOT"
            } else {
                ""
            },
    ...
    fn evaluate<'s>(
        ...
        let result = match clause.access_clause.comparator {
            ...
            (CmpOperator::Eq, not) => match &clause.access_clause.compare_with {
                Some(LetValue::Value(Value::Null)) => match &lhs {
                    None => Some(negation_status(true, not, clause.negation)),
    ...
    for each in expressions {
        match &each.value {
            LetValue::Value(v) => {
                vars.insert(
                    &each.var,
                    PathAwareValue::try_from((v, Path::try_from("rules_file/")?))?,
                );
            }
...
impl<'s, 'loc> RootScope<'s, 'loc> {
    pub(crate) fn new(rules: &'s RulesFile<'loc>, value: &'s PathAwareValue) -> Self {
